...
int main(int argc, char *argv[]) {
	...
	//matrix test
	matrix	*mat4 = new_matrix_glspec();
	matrix	*vec4 = new_matrix_2(4, 1);
	vec4->data[0] = 1.0f; vec4->data[1] = 0.0f; vec4->data[2] = 0.0f; vec4->data[3] = 1.0f;
	float	trans_data[] = { 1.0f, 1.0f, 1.0f };
	matrix_trans(&mat4, trans_data);
	for (int i = 0; i < mat4->rows; ++i) {
		for (int j = 0; j < mat4->columns; ++j) {
			printf("%f ", mat4->data[i * mat4->columns + j]);
		}
		write(1, "\n", 1);
	}
	matrix_multi(&mat4, vec4);
	for (int i = 0; i < mat4->rows; ++i) {
		for (int j = 0; j < mat4->columns; ++j) {
			printf("%f ", mat4->data[i * mat4->columns + j]);
		}
	...
	float scale_data[3] = { 0.5f, 0.5f, 0.5f };
	matrix_rev_z(&mat4, 3 * M_PI / 2);
	matrix_scale(&mat4, scale_data);
	printf("rotate -90 degree and scale 0.5:\n");
	for (int i = 0; i < mat4->rows; ++i) {
		for (int j = 0; j < mat4->columns; ++j) {
			printf("%f ", mat4->data[i * mat4->columns + j]);
		}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUseProgram(prog);
		unsigned int transformLoc = glGetUniformLocation(prog, "transform");
		glUniformMatrix4fv(transformLoc, 1, GL_FALSE, mat4->data);
